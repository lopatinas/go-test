...
APP?=go-test
PROJECT=github.com/lopatinas/go-test
...
GOTEST_LOCAL_HOST?=0.0.0.0
GOTEST_LOCAL_PORT?=8080
GOTEST_LOG_LEVEL?=0
...
run: build
	@echo "+ $@"
	@docker run --name ${CONTAINER_NAME} -p ${GOTEST_LOCAL_PORT}:${GOTEST_LOCAL_PORT} \
		-e "GOTEST_LOCAL_HOST=${GOTEST_LOCAL_HOST}" \
		-e "GOTEST_LOCAL_PORT=${GOTEST_LOCAL_PORT}" \
		-e "GOTEST_LOG_LEVEL=${GOTEST_LOG_LEVEL}" \
